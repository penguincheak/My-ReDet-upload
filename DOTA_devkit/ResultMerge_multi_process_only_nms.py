...
def mergesingle_only_nms(dstpath, nms, nms_thresh, fullname):
    name = util.custombasename(fullname)
    #print('name:', name)
    dstname = os.path.join(dstpath, name + '.txt')
    with open(fullname, 'r') as f_in:
        ...
        for splitline in splitlines:
            ...
            confidence = splitline[1]
            origpoly = list(map(float, splitline[2:]))
            det = origpoly
            det.append(confidence)
            det = list(map(float, det))
            if (oriname not in nameboxdict):
...
def mergebase_parallel(srcpath, dstpath, nms, nms_thresh):
    ...
    # for hrsc2016 and dior
    mergesingle_fn = partial(mergesingle_only_nms, dstpath, nms, nms_thresh)
    # pdb.set_trace()
