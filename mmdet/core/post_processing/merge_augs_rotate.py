...
# change 1
# def merge_rotate_aug_proposals1(aug_proposals, img_metas, rpn_test_cfg):
#     """Merge augmented proposals (multiscale, flip, etc.)
#
#     Args:
#         aug_proposals (list[Tensor]): proposals from different testing
...
#         Tensor: shape (n, 4), proposals corresponding to original image scale.
#     """
#     merged_proposals_list = []
#     for proposals, img_info in zip(aug_proposals[0], img_metas):
#         recovered_proposals = []
...
#         if angle != 0:
#             # TODO: check the angle  负号代表回到之前的角度
#             img_shape = img_info[0]['img_shape_ori']
#             _proposals[:, :4] = bbox_rotate_mapping(_proposals[:, :4], img_shape, -angle)
#         recovered_proposals.append(_proposals)
#         aug_proposal = torch.cat(recovered_proposals, dim=0)
#         merged_proposals, _ = nms(aug_proposal, rpn_test_cfg.nms_thr)
#         scores = merged_proposals[:, 4]
#         _, order = scores.sort(0, descending=True)
#         num = min(rpn_test_cfg.max_num, merged_proposals.shape[0])
#         order = order[:num]
#         merged_proposals = merged_proposals[order, :]
#         merged_proposals_list.append(merged_proposals)
#     return merged_proposals_list
...
def merge_rotate_aug_proposals1(aug_proposals, img_metas, rpn_test_cfg):
    ...
    for proposals, img_info in zip(aug_proposals[0], img_metas):
        ...
        if angle != 0:
            # TODO: check the angle  负号代表回到之前的角度
            _proposals[:, :4] = bbox_rotate_mapping(_proposals[:, :4], img_info[0]['ori_shape'], -angle)
        recovered_proposals.append(_proposals)
    aug_proposal = torch.cat(recovered_proposals, dim=0)
    merged_proposals, _ = nms(aug_proposal, rpn_test_cfg.nms_thr)
    scores = merged_proposals[:, 4]
...
def merge_rotate_aug_bboxes(aug_dbboxes, aug_scores, img_metas, rcnn_test_cfg, index=None):
    ...
    for bboxes, img_info in zip(aug_dbboxes, img_metas):
        ...
        angle = img_info[0]['angle']
        if angle != 0:
            bboxes = dbbox_rotate_mapping(bboxes, img_info[0]['ori_shape'], -angle)
