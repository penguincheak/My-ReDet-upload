...
from ..bbox import bbox_overlaps, rbbox_overlaps_cy_warp, rbbox_overlaps_cy_warp_single
...
def smooth_l1_aug_loss(pred, target, beta=1.0, reduction='mean'):
    assert beta > 0
    assert pred.size() == target.size() and target.numel() > 0

    index = torch.tensor([[1, 0, -1, -2], [-1, -2, -3, 0],
                          [1, 2, 3, 0], [2, 1, 0, -1]]).cuda()
    target_angle = target[:, 4] / 10 - (np.pi / 2)

    zeros = torch.zeros(target.size(0)).cuda()
    ones = torch.ones(target.size(0)).cuda()
    twos = torch.ones(target.size(0)).cuda() * 2
    threes = torch.ones(target.size(0)).cuda() * 3
...
def weighted_smoothl1_aug(pred, target, weight, beta=1.0, avg_factor=None):
    if avg_factor is None:
        avg_factor = torch.sum(weight > 0).float().item() / 4 + 1e-6
    loss = smooth_l1_aug_loss(pred, target, beta, reduction='none')
    return torch.sum(loss * weight)[None] / avg_factor
...
# 不能用iou_loss他的overlaps是4参数的，需要实现一个五参数的overlaps，在geometry.py中找
def iou_smoothl1_loss(pred, target, weight, beta=1.0, bbox_par=None, avg_factor=None):
    inds = torch.nonzero(weight[:, 0] > 0)

    ang = torch.ones(pred.size(0)).cuda()
    ang = ang * (-np.pi/2)
    bbox_par = torch.cat((bbox_par, ang.view(ang.size(0), 1)), 1)
    ...
    assert pred.size() == target.size() and target.numel() > 0
    diff = torch.abs(pred - target)
    reg_loss = torch.where(diff < beta, 0.5 * diff * diff / beta,
                       diff - 0.5 * beta)
    ...
    # means = pred.new_tensor(means).repeat(1, pred.size(1) // 5)
    # stds = pred.new_tensor(stds).repeat(1, pred.size(1) // 5)
    # denorm_pred = pred * stds + means
    # denorm_target = target * stds + means
    #
    # dx = denorm_pred[:, 0::5]
    # dy = denorm_pred[:, 1::5]
    # dw = denorm_pred[:, 2::5]
    # dh = denorm_pred[:, 3::5]
    # dangle = denorm_pred[:, 4::5]
    #
    # Rroi_x = (bbox_par[:, 0]).unsqueeze(1).expand_as(dx)
    # Rroi_y = (bbox_par[:, 1]).unsqueeze(1).expand_as(dy)
    # Rroi_w = (bbox_par[:, 2]).unsqueeze(1).expand_as(dw)
    # Rroi_h = (bbox_par[:, 3]).unsqueeze(1).expand_as(dh)
    # Rroi_angle = (bbox_par[:, 4]).unsqueeze(1).expand_as(dangle)
    #
    ...
    #
    # pred_bbox = torch.stack([gx, gy, gw, gh, gangle], dim=-1).view_as(pred)
    #
    # dx = denorm_target[:, 0::5]
    # dy = denorm_target[:, 1::5]
    # dw = denorm_target[:, 2::5]
    # dh = denorm_target[:, 3::5]
    # dangle = denorm_target[:, 4::5]
    #
    # Rroi_x = (bbox_par[:, 0]).unsqueeze(1).expand_as(dx)
    # Rroi_y = (bbox_par[:, 1]).unsqueeze(1).expand_as(dy)
    # Rroi_w = (bbox_par[:, 2]).unsqueeze(1).expand_as(dw)
    ...
    # gangle = dangle + Rroi_angle
    #
    # target_bbox = torch.stack([gx, gy, gw, gh, gangle], dim=-1).view_as(target)

    pred_bbox = pred + bbox_par
    target_bbox = target + bbox_par

    # 计算iou factor 注意求overlaps的函数用的是numpy，需要用.detach()去掉梯度信息
    pred_d = pred_bbox[inds].detach().cpu().numpy().astype(np.float)
    target_d = target_bbox[inds].detach()
    _ious = rbbox_overlaps_cy_warp_single(pred_d, target_d)  # ious已经在rbbox_overlaps_cy_warp函数中转化为tensor了

    ious = _ious.float() + 1e-5
    # ious出现了0，导致log(ious)成了inf无穷大了所以加1e-5
    reg_loss = reg_loss.sum(dim=1)
    loss = reg_loss / (reg_loss.detach() + 1e-5)

    loss = loss * (-1 * ious.sqrt() * ious.log()).detach()
    # loss = loss * torch.abs((ious * ious + (1. - ious) * reg_loss).log()).detach()
    # loss.sum()
    #  + (1. - _ious)*reg_loss.sum()
    #  + (1. - _ious.float())*torch.abs(reg_loss.sum())
