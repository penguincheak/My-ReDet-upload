...
def dbbox2delta_v2_aug(proposals, gt, means = [0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1]):
    """
        :param proposals: (x_ctr, y_ctr, w, h, angle)
                shape (n, 5)
        :param gt: (x_ctr, y_ctr, w, h, angle)
    ...
    zeros = torch.zeros(gt.size(0)).cuda()
    ones = torch.ones(gt.size(0)).cuda()
    twos = torch.ones(gt.size(0)).cuda() * 2
    threes = torch.ones(gt.size(0)).cuda() * 3

    angle_field = torch.where(gt_angle > 0, ones, zeros)
    angle_field = torch.where((angle_field == 1) & (gt_angle > np.pi / 2), ones, zeros)
    angle_field = torch.where((angle_field == 0) & (gt_angle < (-np.pi / 2)), twos, threes)

    angle_index = index[angle_field.long()]
    angle_aug = angle_index.float() * np.pi / 2
    target_angle = gt_angle.view(gt_angle.size(0), 1).expand_as(angle_aug) \
                    + angle_aug + (np.pi / 2)
    angle_diff = torch.abs(roi_angle.view(target_angle.size(0), 1).expand_as(target_angle)
                           - target_angle)
    angle_diff = torch.min(angle_diff, dim=1).values

    targets_dangle = angle_diff
    dist = targets_dangle % (2 * np.pi)
    dist = torch.min(dist, np.pi * 2 - dist)
    try:
        assert np.all(dist.cpu().numpy() <= (np.pi / 2. + 0.001))
...
def best_match_dbbox2delta(Rrois, gt, means = [0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1]):
    ...
    # bbox_targets = dbbox2delta_v2_aug(Rrois, gt_boxes_new, means, stds)
...
def dbbox_mapping(dbboxes, img_shape, scale_factor, flip):
    ...
    if isinstance(scale_factor, np.ndarray):
        new_dbboxes[..., 0::5] = dbboxes[..., 0::5] * dbboxes[..., 0::5].new_tensor(scale_factor)
        new_dbboxes[..., 1::5] = dbboxes[..., 1::5] * dbboxes[..., 1::5].new_tensor(scale_factor)
        new_dbboxes[..., 2::5] = dbboxes[..., 2::5] * dbboxes[..., 2::5].new_tensor(scale_factor)
        new_dbboxes[..., 3::5] = dbboxes[..., 3::5] * dbboxes[..., 3::5].new_tensor(scale_factor)
    else:
...
def dbbox_mapping_back(dbboxes, img_shape, scale_factor, flip):
    ...
    if isinstance(scale_factor, np.ndarray):
        new_dbboxes[..., 0::5] = new_dbboxes[..., 0::5].view(-1, 2) / new_dbboxes[..., 0::5].new_tensor(scale_factor)
        new_dbboxes[..., 1::5] = new_dbboxes[..., 1::5].view(-1, 2) / new_dbboxes[..., 1::5].new_tensor(scale_factor)
        new_dbboxes[..., 2::5] = new_dbboxes[..., 2::5].view(-1, 2) / new_dbboxes[..., 2::5].new_tensor(scale_factor)
        new_dbboxes[..., 3::5] = new_dbboxes[..., 3::5].view(-1, 2) / new_dbboxes[..., 3::5].new_tensor(scale_factor)
    else:
