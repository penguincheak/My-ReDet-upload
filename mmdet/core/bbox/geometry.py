...
def rbbox_overlaps_cy_warp_single(rbboxes, query_boxes):
    # TODO: first calculate the hbb overlaps, for overlaps > 0, calculate the obb overlaps
    # import pdb
    # pdb.set_trace()
    box_device = query_boxes.device
    ...
    # # pdb.set_trace()
    # inds = np.where(ious > 0)
    ious = np.zeros(polys_np.shape[0])
    for index in range(polys_np.shape[0]):

        box = polys_np[index]
        query_box = query_polys_np[index]
