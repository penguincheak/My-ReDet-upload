...
class Bottleneck(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        self.CHAttention = ChannelAttention(self.mid_channels)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            att = self.CHAttention(out)

            out.tensor = out.tensor * att
...
class ReResNet_OAtt(BaseBackbone):
    """ReResNet backbone.
    ...
    def __init__(self,
        ...
        super(ReResNet_OAtt, self).__init__()
        ...
        # self.channelAtt = eca_layer(stem_channels / Orientation)
        # self.channelAtt = ReSElayer(stem_channels, stem_channels)
    ...
    def init_weights(self, pretrained=None):
        super(ReResNet_OAtt, self).init_weights(pretrained)
    ...
    def forward(self, x):
        if not self.deep_stem:
            ...
            x = self.norm1(x)
            x = self.relu(x)
            # x size是64 512 512只缩小了1倍
        # 通道注意力和方向通道注意力
        # 先试通道注意力
        # feat = x.tensor
        # feat = feat[:, 0:int(self.stem_channels / Orientation), :, :]
        # ch_att = self.channelAtt(feat).expand_as(feat)
        # feat = ch_att
        # for j in range(1, 8):
        #     feat = torch.cat((feat, ch_att), dim=1)
        # x.tensor = x.tensor * feat

        # maxpooling之后可能会损失空间信息

        # feat = self.channelAtt(x)
        # x.tensor = x.tensor * feat.tensor

        x = self.maxpool(x)
        # x是64 256 256，进入reslayer后变为256 256 256
    ...
    def train(self, mode=True):
        super(ReResNet_OAtt, self).train(mode)
        self._freeze_stages()
