...
from ..losses.attention_loss_ssp import AttentionLoss_SSP


@DETECTORS.register_module
class SSPReDet(BaseDetectorNew, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(SSPReDet, self).__init__()

        self.backbone = builder.build_backbone(backbone)

        if neck is not None:
            self.neck = builder.build_neck(neck)
            self.attention_loss = AttentionLoss_SSP()
    ...
    def init_weights(self, pretrained=None):
        super(SSPReDet, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        attention_loss = self.attention_loss([img.size(2), img.size(3)],
                att_heatmaps, gt_bboxes, self.alpha, self.beta)

        attention_loss = dict(attention_loss=attention_loss)
        losses.update(attention_loss)
