...
from ..plugins.heatmap import Heatmap


@DETECTORS.register_module
class SSReDet_2(BaseDetectorNew, RPNTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        assert backbone['type'] == 'ReResNet' or 'SEResNet', 'ReDet only supports ReResNet backbone'
    ...
    def init_weights(self, pretrained=None):
        super(SSReDet_2, self).init_weights(pretrained)
    ...
    def extract_feat(self, img):
        ...
        # Get SSP Features
        if self.with_neck:
            x, att_heatmaps = self.neck(x)
        return x, att_heatmaps
    ...
    def forward_train(self,
                      ...
                      gt_masks=None,
                      proposals=None):
        x, att_heatmaps= self.extract_feat(img)
        ...
        # _gt0 = gt_masks[0][0, :, :]
        # for i in range(gt_masks[0].shape[0]):
        #     _gt0 += gt_masks[0][i, :, :]
        # _gt0[_gt0 > 0] = 1
        # plt.matshow(_gt0, cmap="gray")
        # plt.draw()
        # plt.savefig("/home/penguin/feature_maps/gt0.jpg")
        # plt.close()
        #
        # _gt1 = gt_masks[1][0, :, :]
        # for i in range(gt_masks[1].shape[0]):
        #     _gt1 += gt_masks[1][i, :, :]
        # _gt1[_gt1 > 0] = 1
        # plt.matshow(_gt1, cmap="gray")
        # plt.draw()
        # plt.savefig("/home/penguin/feature_maps/gt1.jpg")
        # plt.close()
        # #---------------------------

        gt_reg = self.att_loss.target(att_heatmaps, gt_bboxes)

        # #--------------------------------
        # for i in range(len(gt_reg)):
        #
        #     gt0 = gt_reg[i].detach().cpu()
        #     _gt0 = gt0[0, 0, :, :]
        #     _gt0[_gt0 < 0] = 0
        #     plt.matshow(_gt0, cmap="gray")
        #     plt.draw()
        #     plt.savefig("/home/penguin/feature_maps/gt_reg/reg0_{}.jpg".format(i))
        #     plt.close()
        #
        #     _gt1 = gt0[1, 0, :, :]
        #     _gt1[_gt1 < 0] = 0
        #     plt.matshow(_gt1, cmap="gray")
        #     plt.draw()
        #     plt.savefig("/home/penguin/feature_maps/gt_reg/reg1_{}.jpg".format(i))
        #     plt.close()
        # # --------------------------------

        loss_att = self.att_loss.loss(reg_pred=att_heatmaps, reg_gt=gt_reg)
