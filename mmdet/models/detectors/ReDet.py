...
class ReDet(BaseDetectorNew, RPNTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            # rbbox_targets 是看水平pos_proposal和gt的差别，
            # 角度水平默认为-pi / 2, 中心点dx,dy dw,dh dtheta
            rbbox_targets = self.bbox_head.get_target(
                sampling_results, gt_masks, gt_labels, self.train_cfg.rcnn[0])

            bbox_par = torch.stack([res.bboxes for res in sampling_results])
            bbox_par = bbox_par.view(bbox_par.size(0) * bbox_par.size(1), bbox_par.size(-1))
            bbox_par = None
            loss_bbox = self.bbox_head.loss(cls_score, bbox_pred, *rbbox_targets, bbox_par)
    ...
    def aug_test(self, imgs, img_metas, rescale=None):
        ...
        for x, img_meta in zip(self.extract_feats(imgs), img_metas):
            ...
            proposals_scale = bbox_mapping(proposal_list[:, :4], img_shape, scale_factor, flip)
            if angle == 0:
                proposals = proposals_scale
            else:
                proposals = bbox_rotate_mapping(proposals_scale, img_shape, angle)
