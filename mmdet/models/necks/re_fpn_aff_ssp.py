...
class ReASPP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dilations=(1, 2, 5, 1)):
        ...
        self.gap = ennAdaptiveAvgPool(inplanes=in_channels, outputsize=1)
...
class ReFPN_AFF_SSP(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 activation=None):
        super(ReFPN_AFF_SSP, self).__init__()
            ...
            for k in range(Orientation):
                for j in range(self.out_channels // Orientation):
                    if j == 0 and k == 0:
                        ...
                        att_insec = torch.cat((att_insec,
                                               att_2x[:, k, :, :].reshape(att_2x.shape[0], 1, att_2x.shape[2],
                                                                          att_2x.shape[3])), 1)

            y.tensor = att_insec * y.tensor
            wei = self.iAFF_list[i](x, y)

            laterals[i - 1].tensor = x.tensor * wei + y.tensor * (1 - wei)

            # import matplotlib.pyplot as plt
            # from torchvision.utils import save_image
            # import os
            # if not os.path.exists('/home/penguin/feature_maps/{}'.format(i)):
            #     os.mkdir('/home/penguin/feature_maps/{}'.format(i))
            # featmp = feat
            # featmp = featmp.detach().cpu()
            # for cc in range(featmp.size(1)):
            #     # save_image(featmp[0,cc,:,:], os.path.join('/home/penguin/feature_maps/{}'.format(i), 'image_{}.jpg'.format(cc)),
            #     #            nrow=1, padding=0)
            #     plt.matshow(featmp[0, cc, :, :], cmap="viridis")
            #     plt.draw()
            #     plt.savefig('/home/penguin/feature_maps/{}'.format(i) + '/image_{}.jpg'.format(cc))
            #     plt.close()
