...
class ReInceptionModule(enn.EquivariantModule):
    def __init__(self, inchannels, outchannels):
        super(ReInceptionModule, self).__init__()
        self.outchannels = outchannels

        self.conv_3x3_1 = convnxn(inchannels, outchannels, kernel_size=3, stride=1, padding=1, dilation=1)
        self.bn_conv_3x3_1 = ennBatchNorm(outchannels)

        self.conv_3x3_2 = convnxn(inchannels, outchannels, kernel_size=3, stride=1, padding=1)
        self.bn_conv_3x3_2 = ennBatchNorm(outchannels)
        self.relu_3x3_2 = ennReLU(outchannels)
        self.conv_3x3_2_i = convnxn(inchannels, outchannels, kernel_size=3, stride=1, padding=3, dilation=3)
        self.bn_conv_3x3_2_i = ennBatchNorm(outchannels)

        self.conv_3x3_3 = convnxn(inchannels, outchannels, kernel_size=3, stride=1, padding=1)
        self.bn_conv_3x3_3 = ennBatchNorm(outchannels)
        self.relu_3x3_3 = ennReLU(outchannels)
        self.conv_3x3_3_i = convnxn(inchannels, outchannels, kernel_size=3, stride=1, padding=5, dilation=5)
        self.bn_conv_3x3_3_i = ennBatchNorm(outchannels)

        self.conv_1x1 = convnxn(outchannels * 3, outchannels, kernel_size=1, stride=1)
        self.bn_conv_1x1 = ennBatchNorm(outchannels)
        self.relu = ennReLU(outchannels)

    def forward(self, x):
        conv1 = self.bn_conv_3x3_1(self.conv_3x3_1(x)).tensor
        conv2 = self.bn_conv_3x3_2_i(self.conv_3x3_2_i(self.relu_3x3_2(
            self.bn_conv_3x3_2(self.conv_3x3_2(x))
        ))).tensor
        conv3 = self.bn_conv_3x3_3_i(self.conv_3x3_3_i(self.relu_3x3_3(
            self.bn_conv_3x3_3(self.conv_3x3_3(x))
        ))).tensor
        conv_out = torch.cat((conv1, conv2, conv3), 1)
        conv_out = enn.GeometricTensor(conv_out, FIELD_TYPE['regular'](gspace, self.outchannels * 3))
        conv_out = self.bn_conv_1x1(self.conv_1x1(conv_out))

        conv_out = self.relu(conv_out + x)
        return conv_out
...
class ReFPN_Inception(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 activation=None):
        super(ReFPN_Inception, self).__init__()
        ...
        self.inception = ReInceptionModule(out_channels, out_channels)
    ...
    def forward(self, inputs):
        ...
        laterals[0] = self.inception(laterals[0])
