...
class Re_G_and_L_Att(nn.Module):
    '''
    多特征融合 iAFF
    '''

    def __init__(self, channels=256, r=2):
        super(Re_G_and_L_Att, self).__init__()
    ...
    def forward(self, x):
        xl = self.local_att_bn2(self.local_att_conv2(self.local_att_relu(
            self.local_att_bn1(self.local_att_conv1(x))
        )))
        xg = self.global_att_bn2(self.global_att_conv2(self.global_att_relu(
            self.global_att_bn1(self.global_att_conv1(self.global_pool(x)))
        )))
        xlg = xl + xg
...
class Re_Att_SSP(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 activation=None):
        super(Re_Att_SSP, self).__init__()
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            channel_att = Re_G_and_L_Att(out_channels)
    ...
    def forward(self, inputs):
        ...
        for i in range(0, len(laterals)):
            att = self.channel_attentions[i](laterals[i])
