...
class ReSEP(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale_equal_conv_high = nn.ModuleList()
        self.scale_equal_conv_mid = nn.ModuleList()
        self.scale_equal_conv_low = nn.ModuleList()
        for i in range(self.start_level, self.backbone_end_level):
            self.scale_equal_conv_high.append(ConvModule(
                out_channels,
                out_channels,
                3,
                padding=1,
            ...
            self.scale_equal_conv_mid.append(ConvModule(
                out_channels,
                out_channels,
                3,
                padding=1,
            ...
            self.scale_equal_conv_low.append(ConvModule(
                out_channels,
                out_channels,
                3,
                padding=1,
    ...
    def forward(self, inputs):
        ...
        # build scale equalizing path
        laterals_next = []
        # 特征融合
        for level, feature in enumerate(laterals):
            tem_feat = self.scale_equal_conv_mid[1](feature)
            if level > 0:
                tem_feat += self.scale_equal_conv_low[2](laterals[level - 1])
            if level < len(laterals) - 1:
                tem_feat += self.up_samples[level]((self.scale_equal_conv_high[0](laterals[level + 1])))
            laterals_next.append(tem_feat)
        ...
        outs = [
            self.fpn_convs[i](laterals_next[i]) for i in range(used_backbone_levels)
        ]
