...
class ReSSP_2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        _laterals = [
            lateral.tensor for lateral in laterals
        ]

        att_heatmaps = self.CAM(_laterals)

        for i in range(self.start_level, self.backbone_end_level):
            laterals[i].tensor = (1 + att_heatmaps[i]) * laterals[i].tensor
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            att_2x = F.interpolate(att_heatmaps[i], size=prev_shape)
            att_insec = att_heatmaps[i - 1] * att_2x
