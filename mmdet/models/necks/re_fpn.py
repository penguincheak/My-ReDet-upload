...
class ReFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for k in range(len(laterals)):
            tensor = laterals[k].tensor

            # feature_map = tensor.detach().cpu()
            #
            # for i in range(feature_map.size(1)):
            #     plt.matshow(feature_map[0, i, :, :], cmap=plt.get_cmap('gist_heat')) #"viridis"
            #     plt.savefig(os.path.join('/home/penguin/可视化result/{}/'.format(k + 2), 'image_{}.jpg'.format(i)))

            for i in range(tensor.size(1)):
                save_image(tensor[0, i, :, :], os.path.join('/home/penguin/可视化result/{}/'.format(k + 2), 'image_{}.jpg'.format(i)),
                           nrow=1, padding=0)
