...
from mmdet.core import accuracy

from .cross_entropy_loss_new import cross_entropy
...
def seesaw_ce_loss(cls_score,
    ...
    cls_score = cls_score + (seesaw_weights.log() * (1 - onehot_labels).float())

    loss = F.cross_entropy(cls_score, labels, weight=None, reduction='none')

    if label_weights is not None:
        loss = loss * label_weights.float()
